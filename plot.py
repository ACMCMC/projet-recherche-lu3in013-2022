...
class CombinedGraphMaker :
        ...
        def load(self,file):
                with open(file,'r') as myfile:
                        ...
                        size = len(list(json_data.items())[0][1]) #You get the size of the population from the first item of the json file
        ...
        def get_all_rewards(self,data):
                ...
                for timestep in data.keys():
                        timestep_rewards = torch.tensor([agent['reward']  for agent in data[timestep].values()])
                        all_rewards=torch.cat((all_rewards, timestep_rewards.unsqueeze(0)))
                return all_rewards   
        ...
        def get_all_hyperparam_values(self, data, hyperparam):
                all_values = torch.tensor([])
                for timestep in data.keys():
                        timestep_values = torch.tensor([agent['hyperparameters'][hyperparam] for agent in data[timestep].values()])
                        all_values=torch.cat((all_values, timestep_values.unsqueeze(0)))
                return all_values
        ...
        def plot_rewards_mean_and_individuals(self, line_color='blue'):
                for i in self.data.keys():
                        ...
                        mean_rewards = all_rewards.mean(1)
                        agents = range(all_rewards.size(1))
                        timesteps = [int(t) / len(agents) for t in data_i.keys()]  # Adjust the timesteps, to show the number of timesteps that each agent has done
                        for a in agents:
                                plt.plot(timesteps, all_rewards.select(1, a), color='grey', alpha=0.3, linewidth=1)
                                #plt.scatter(timesteps, mean_rewards, color=line_color)
                        plt.plot(timesteps, mean_rewards, color=line_color, linewidth=2, label=i)
                plt.legend()
                self.ax.set(xlabel='timestep',ylabel='reward', title='Combinaison des évolutions individuelles de la récompense')
        ...
        def plot_rewards_mean_and_std(self, line_color='blue'):
                for i in self.data.keys():
                        ...
                        plt.plot(timesteps, mean_rewards, color=line_color, linewidth=2,label=i)
                plt.legend()
                self.ax.set(xlabel='timestep', ylabel='reward', title='Moyenne et écart type combinés des récompenses')        
        ...
        def plot_hyperparam_individuals(self, hyperparam, line_color='blue'):
                for i in self.data.keys():
                        ...
                        for a in agents:
                                plt.plot(timesteps, all_values.select(1, a), color='grey', alpha=0.3, linewidth=1)
                                #plt.scatter(timesteps, mean_values, color=line_color)
                        plt.plot(timesteps, mean_values, color=line_color, linewidth=2,label=i)
                plt.legend()
                self.ax.set(xlabel='timestep', ylabel=hyperparam, title='Combinaison des évolutions individuelles de ' + hyperparam)
        ...
        def plot_hyperparam_mean_and_std(self, hyperparam, line_color='blue'):
                for i in self.data.keys():
                        ...
                        plt.plot(timesteps, mean_values, color=line_color, linewidth=2,label=i)
                plt.legend()
                self.ax.set(xlabel='timestep', ylabel=hyperparam, title='Moyenne et écart type combinés de ' + hyperparam)
...
if __name__ == "__main__":
    ...
    #logger = CustomLogger()
    #logger.open('./raw_data/{}'.format(filename))
    ...
    for i in range(5):
        print(filelist[i])
        graphmaker.load('./raw_data/{}'.format(filelist[i]))
        
    graphmaker.start_plot()
    graphmaker.plot_rewards_mean_and_individuals()
    graphmaker.end_plot(output_dir + 'combined_reward_individuals.png')
    graphmaker.start_plot()
    graphmaker.plot_rewards_mean_and_std()
    graphmaker.end_plot(output_dir + 'reward_std.png')
    graphmaker.start_plot()
    graphmaker.plot_hyperparam_individuals('a2c_coef', line_color='green')
    graphmaker.end_plot(output_dir + 'a2c_coef_inds.png')
    graphmaker.start_plot()
    graphmaker.plot_hyperparam_mean_and_std('a2c_coef', line_color='green')
    graphmaker.end_plot(output_dir + 'a2c_coef_mean_and_std.png')
    graphmaker.start_plot()
    graphmaker.plot_hyperparam_individuals('critic_coef', line_color='red')
    graphmaker.end_plot(output_dir + 'critic_coef_inds.png')
    graphmaker.start_plot()
    graphmaker.plot_hyperparam_mean_and_std('critic_coef', line_color='red')
    graphmaker.end_plot(output_dir + 'critic_coef_mean_and_std.png')
    graphmaker.start_plot()
    graphmaker.plot_hyperparam_individuals('entropy_coef', line_color='orange')
    graphmaker.end_plot(output_dir + 'entropy_coef_inds.png')
    graphmaker.start_plot()
    graphmaker.plot_hyperparam_mean_and_std('entropy_coef', line_color='orange')
